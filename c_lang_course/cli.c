...
#define CLI_HISTORY_FILE ".history"
...
static const char *gen_cmd(void) {
    while (commands[gen_idx]) {
        ...
        if (strncasecmp(name, gen_text, gen_len) == 0)
            return name;
    }
    return NULL;
}
...
static unsigned char complete(EditLine *e, int ch) {
    ...
    if (strlen(first) > gen_len)
        el_insertstr(e, first + gen_len);
...
Cli *cli_create(void) {
    ...
    history(cli->hist, &ev, H_LOAD, CLI_HISTORY_FILE);
...
int cli_run(Cli *cli, void *ctx, CliOnLine on_line) {
    while (1) {
        ...
        if (s[0] != '\0') {
            ...
            history(cli->hist, &ev, H_SAVE, CLI_HISTORY_FILE);
        }
